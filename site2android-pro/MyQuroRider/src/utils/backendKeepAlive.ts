 ...
 * This ensures the Render container stays warm and avoids cold start delays for riders.
...
export async function pingBackend(backendUrl: string): Promise<void> {
  try {
    ...
    if (response?.ok) {
      console.log('🟢 [KeepAlive] Rider backend warm & responsive');
    }
  } catch {
...
/**
 * Initializes the keep-alive monitor for rider app:
 * 1. Pings immediately on app launch.
 * 2. Runs periodic heartbeat while the app is active.
 * 3. Wakes up backend whenever rider app returns from background.
 */
